...
const UpdateVIR = ({navigation, route}) => {
  ...
  const GET_PROFILE = () => {
    ...
    GET_WITH_TOKEN(
      ...
      response => {
        ...
        setLoading(false);
        setGetImage(response?.data?.driver_details?.vehicle_inspection_report);
  ...
  return (
    <ContainerView adjust={10}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          <View style={{paddingHorizontal: 10}}>
            <MainTitle
              back={true}
              boldTitle={true}
              toptitle={localization?.UpdateVIR?.header}
              subTitleStyle={{marginTop: 16}}
              subTitle={localization?.UpdateVIR?.sub_header}
            />
            <WarningComponent
      ...
      <Button
        ...
        onPress={() => {
          onSubmit('vehicle-inspection-expiry-date');
        }}
        loading={loading}
